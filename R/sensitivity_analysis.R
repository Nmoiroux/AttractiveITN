...
library(pse) # as pse in no longer on CRAN, use comment lines above to install the last CRAN version
library(sensitivity)
library(tidyverse)
library(ggpubr)
library(ggbeeswarm)
...
annotate_figure(figure_pse,
								bottom = text_grob("% reduction in transmission potential")#,
...
dat <- dat %>% cbind(res) %>% mutate(g = jitter(g), n = jitter(n)) ### ATENTION au JITTER
dat_plot <- dat %>% pivot_longer(cols = c(1:14), names_to = "var") %>% mutate(var = as.factor(var)) %>%
	filter(var %in% c("Du", "g","Ih","k", "m1u", "m2p", "m2u", "n", "Nh", "S")) %>%
	mutate(var = fct_drop(var))

ggplot(dat_plot, aes(x=value, y=res)) + 
	facet_wrap(~var,scales = "free", drop=FALSE)+
	geom_point(size = 0.5) +
	ylab("% reduction in transmission potential") +
	geom_smooth()
...
# sobol indices
x <-sensitivity::fast99(modelRun,  factors, n=1000, q=q, q.arg=q.arg)
plot(x)
print(x)
...
dat_x <- x$X
res_x <- x$y
dat_x <- dat_x %>% cbind(res_x) %>% mutate(g = jitter(g), n = jitter(n)) ### ATENTION au JITTER (for graphic)
dat_x_plot <- dat_x %>% pivot_longer(cols = c(1:14), names_to = "var") %>% mutate(var = as.factor(var)) 

dat_x_plot %>%
	filter(var %in% c("Du", "g","Ih","k", "m1u", "m2p", "m2u", "n", "Nh", "S")) %>%
	mutate(var = fct_drop(var)) %>%
	ggplot(aes(x=value, y=res_x)) + 
	facet_wrap(~var,scales = "free", drop=FALSE)+
	geom_point(size = 0.1) +
	ylab("% reduction in transmission potential") +
	geom_smooth()
