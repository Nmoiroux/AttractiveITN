...
predict_kdr <- function(q=0.1, W_F=c(1,1,1), W_M=c(1,1,1), N.gen=100){
  ...
  return(list(fkdr,pop)) 
}

predict_kdr(W_F=c(1,0.8,0.66), W_M=c(0.67,1,0.55), N.gen=100)[[1]]

#' `predic-_kdr4` is a one-locus diploid selection model. It
#' simulates the dynamic of the allelic frequency of a mutation, 
#' allowing for sex differential selection.
#' 
#'
...
#'
#' @examples
#' predict_kdr4(W_F=c(1,0.8,0.66), W_M=c(0.67,1,0.55), N.gen=100)[[1]]
predict_kdr4 <- function(q=0.1, W_F=c(1,1,1), W_M=c(0.67,1,0.55), N.gen=100, sex.ratio = 1){
  
  # initialize first population (Hardy-Weinberg equilibrium)
  p <- 1 - q
  RRf <- q^2    # frequency RR females 
  SSf <- p^2    # frequency SS females
  RSf <- 2*p*q  # frequency RS females
  
  RRm <- RRf		# frequency RR males
  SSm <- SSf		# frequency SS males
  RSm <- RSf		# frequency RS males
  ...
  for(i in 2:N.gen){
    ...
    # next generation normalized genotype frequencies per sex
    
    RRf2 <- mRRf / (mRRf + mSSf + mRSf)	#frequency RR females
    SSf2 <- mSSf / (mRRf + mSSf + mRSf)	#frequency SS females
    RSf2 <- mRSf / (mRRf + mSSf + mRSf)	#frequency RS females
    
    RRm2 <- mRRm / (mRRm + mSSm + mRSm)	#frequency RR males
